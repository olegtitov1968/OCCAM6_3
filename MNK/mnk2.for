...
C   ESPECIFICATIONS
      ...
      DIMENSION number(nss), ra(nss), de(nss), rbar(3) 
     ...
     *       corr(npar*(npar+1)/2), eps1(nobs1), eps2(nobs1)
      ...
      dimension p1(nobs1), p2(nobs1), dd2(nmax), chik2(nmax) 
      dimension xbar(nobs1), ybar(nobs1), zbar(nobs1)
...
c
      ...
      if (num_unst.eq.7970)OPEN(24,FILE='arc_7970.CAT',status='unknown')  ! 
 
      if (num_unst.eq.8720)open(24,file='arc_8720.cat',status='unknown')  !  aus2024b
  
      if (num_unst.ne.11.and.num_unst.ne.163.and.num_unst.ne.123.
     *  and.num_unst.ne.57.and.num_unst.ne.87.and.num_unst.ne.7970.
     *  and.num_unst.ne.102.and.num_unst.ne.14.and.num_unst.ne.1495.
     *  and.num_unst.ne.1591.and.num_unst.ne.8720.and.num_unst.ne.8014)
...
c
     ...
     * file = 'STATIST_OF_BAD_SESSIONS.DAT', status='unknown')    !  1-April-2016
      ...
      open (88, file='baseline.dat',status='unknown')
c
c       open (332,file='scan_length.dat',status='unknown')
...
c      print *, idat_1, ut1_1/twopi
 ...
 445  continue
      ...
      DO K = 1,num_unst
         ...
         if (num_unst.eq.8720.or.num_unst.eq.7970) then 

             read (24,125) icrf107(k)

         else

             read (24,225) icrf107(k)
...
c-  OT  04.06.2010
     ...
     *   thdelay, freq0, z31, z32, parallax, rbar )     !        26-June-2015, thdelay added on 6-Nov-2015 for software cross-check, 19-Sep-2019
...
c
         	                 ...
         	                 H2 (IROW + (NR1-(3*i+8))*nfull) = 0.d0      !  clock offset         
                                 H2 (IROW + (NR1-(3*i+7))*nfull) = 0.d0      !  clock rate
                                 H2 (IROW + (NR1-(3*i+6))*nfull) = 0.d0      !  clock 2nd rat
...
c
           ...
           H2 (IROW + (NR1-(3*i+8))*nfull) = ISIG/(100.d0*c)             !  [sec/cm]clock offset
           H2 (IROW + (NR1-(3*i+7))*nfull) = isig*dt(irow)/(100.d0*c)    !  clock rate
           H2 (IROW + (NR1-(3*i+6))*nfull) = isig*dt(irow)**2/(100.d0*c) !  clock 2nd rate
...
444   format (' The number of sources in the session is ',i3,';'2x,i4,'
 ...
 87   format (1x,a8,4x,i4,2(3x,f8.4))
     ...
     *      (i_st1(i).eq.3.and.i_st2(i).eq.2))  then
             ...
             write (88,589) year(i), c * eps1(i), c * z2(i), iis(i),   
     *      antenna(i_st1(i)), antenna(i_st2(i)), source(i)
     ...
     *                         y, chi1, sd, eps2, s0, s1, covm) 
         ...
         s1 = s1 / ((100.d0 * c )**2) !  s1 converted from [1/ps^2] to [1/cm^2]
...
c
         ...
         xbar0 = xbar(1) + (xbar(nf) - xbar(1))/2.d0
         ybar0 = ybar(1) + (ybar(nf) - ybar(1))/2.d0
         zbar0 = zbar(1) + (zbar(nf) - zbar(1))/2.d0

         print *, xbar(1), ybar(1), zbar(1)
         print *, xbar(nf), ybar(nf), zbar(nf)
         print *, xbar0, ybar0, zbar0
     ...
     *      
     *      numob(i), ijj, time_m,
     *      iy1, mo, idd, dbc_code, xbar0, ybar0, zbar0                   !  07.11.2008
...
126   format (f12.5,2(2x,f14.8), 2x,a8,6(2x,f18.9),2x,f9.5,                !   TETA added on 22 May, 2014
     *2(2x,i4),2x,f11.5,1x,i2.2,a3,i2.2,a2,3(2x,f18.4))                       !  13.05.2025
...
C**********************************************************************
     ...
     *   CMODEL,  part_gamma,
     * teta, bb, thdelay, 
     * freq0, z31, z32, parallax, rbar)      !   19-Sep-2019
...
C   ESPECIFICATIONS
      ...
      dimension acc_der(3), ww1(3), ww2(3), p(3), b(3), bsw(3), rbar(3)
