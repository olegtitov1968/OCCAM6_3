...
C
      ...
      nr1 = nr +  (nantenna-1) *( nantenna-2)/2
      ...
      ELSE
         ...
         NR1 = NR1 + 8   ! for nutation,EOP and rates
...
C**********************************************************************
C
C    SUBROUTINE prepare (cmodel, nr1, nr)
C
...
C**********************************************************************
      ...
      SUBROUTINE prepare (cmodel, nr1, nr)
...
C   ESPECIFICATIONS
     ...
     *       corr(npar*(npar+1)/2), eps1(nobs1), eps2(nobs1)
      dimension y(npar), sy(npar), a2(nobs1*npar)
...
c
            ...
            if (i_st1(irow) .gt.1.and.i_st2(irow).gt.1) then             
 
               if (i_st1(irow).ge.i_st2(irow)) then

                    ij = (i_st1(irow)-1)*(i_st1(irow)-2)/2 + 
     *  (i_st2(irow)-1) - (i_st1(irow)-2) 

               else

                    ij = (i_st2(irow)-1)*(i_st2(irow)-2)/2 + 
     *  (i_st1(irow)-1) - (i_st2(irow)-2)  

               end if

c               print *, i_st1(irow)-1, i_st2(irow)-1, 
c     * antenna(i_st1(irow)), antenna(i_st2(irow)), ij
              
               h2(irow + (nr+ij-1)*nfull) = 1.d0
...
c 777    format (200(1x,f18.12))
 778   format (f16.8,3(2x,a8))
      ...
      call mnk4 (nr1, nr2, nfull, year, r2, z2, h2, 
     *                    y, schi, sw, eps1, s00, s11, covm1, a2)

      print *, ' first mnk is finished '
...
444   format (' The number of sources in the session is ',i3,';'2x,i4,'
 ...
 87   format (1x,a8,4x,i4,2(3x,f8.4))
     ...
     *      antenna(i_st1(i)), antenna(i_st2(i)), source(i)
         ...
         call mnk1 (nr1, nr2, nfull, year, r2, z2, a2, 
