...
C   ESPECIFICATIONS
      ...
      DIMENSION p1(nobs1), p2(nobs1)     

      DIMENSION TIM(nobs1),IDICTIO(nobserv),IOBS(nobserv)
      DIMENSION PCOORD(2,8),PCOORR(2,8),PNUT(2),PPOL(5), PSOU(2)
...
c
     ...
     * file = 'STATIST_OF_BAD_SESSIONS.DAT', status='unknown')    !  1-April-2016
      ...
      open (87, access='append', file='resid.dat',status='unknown')
...
c      print *, ' tt = ', tt
     ...
     *          hsgp1, hp1, hsgm1, hm1, hsg, h, omega(irow)
         ...
         CALL ICRF2000 (source(irow), ng, isor, ra2000, de2000,
     *                            fulsor, num, in, ra, de)
...
c         print *, eps1(i), dsqrt(r2(i))

         if ((eps1(i)**2/r2(i)).gt.tstfac*sw) then
...
c
 ...
 103  format (f11.4,1x,i5,2(2x,f8.4))
          ...
          omee = dsqrt(y(nr1-2)**2 + y(nr1-1)**2)
          s_omee = dsqrt(sy(nr1-2)**2 + sy(nr1-1)**2)
     ...
     *  omee, s_omee, omega1, omega1 - omega_s, day_length,
     *  iy1, mo, idd, dbc_code, year_1 

 105  format (f12.5,11(1x,f14.4),2x,i2.2,a3,i2.2,a2,2x,f12.5)
...
C   GET PARTIAL DERIVATIVES WITH RESPECT TO ROTATION
      ...
      PSOU (1) =  (PCOORR(2,4)-PCOORR(1,4))/c     !  m/s -> rad
      PSOU (2) =  (PCOORR(2,5)-PCOORR(1,5))/c     !  m/s -> rad
