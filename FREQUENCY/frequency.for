...
C   ESPECIFICATIONS
      ...
      DIMENSION PCOORD(2,8),PCOORR(2,8),PNUT(2),PPOL(5)
...
c      call mnk1 (nr3, nr3, nfull, year, r2, z1, h1, schi1, sw1, eps2,
c     *  x, sx, syst1, corr1)
   
c       do i=1,nf

c         if ((antenna(i_st1(i)).eq.'BR-VLBA '.and.antenna(i_st2(i)).eq.
c     *                                                    'LA-VLBA ').
c     * or.(antenna(i_st2(i)).eq.'BR-VLBA '.and. antenna(i_st1(i)).eq.
c     * 'LA-VLBA ')) then
  

c             if (SOURCE(i).eq.'0552+398') then


c              call omega_tide (year(i), cor1)

c              cor1 = -cor1 * 1.d12

c              write (87,589) year(i), eps1(i), z2(i), r2(i),
c    *   year(i)-dble(int(year(i))),
c     *   antenna(i_st1(i)), antenna(i_st2(i)), source(i), bb(i)

c              write (77,*) year(i), dww1(i), dww2(i), dww3(i),
c     * p1(i), p2(i), p3(i), 1.d6 * dwp(i), freq23(i)
...
c  589  format (f12.6,4(2x,f13.4),3(2x,a8),2x,f12.2)
...
C   ESPECIFICATIONS
      ...
      DIMENSION PEOP1(3,3),PEOP2(3,3),PPOL(5)
      DIMENSION PPOL1(3)
     ...
     * a1(3), a2(3), p1(3), da(3)
      ...
      DIMENSION azimut1(3), azimut2(3), qu(3), btr(3)

      dimension trf1(3), trf2(3)      
...
C   AND THE AXIS OFFSETS
     ...
     *            , qu(1), qu(2), qu(3)     ! source vector in TRS
     *            , trf1(1), trf1(2), trf1(3)     ! source vector in TRS
     ...
     *            , qu(1), qu(2), qu(3)     ! source vector in TRS
     *            , trf2(1), trf2(2), trf2(3)     ! source vector in TRS
...
c
	...
	dvd=0.d0
      ...
      btr(1) = trf2(1) - trf1(1)   !  baseline vector in TRS
      btr(2) = trf2(2) - trf1(2)
      btr(3) = trf2(3) - trf1(3)
...
c
c       Partial derivatives in TRS
c

c      PPOL(1) =  - (Btr(2) * qu(3) - Btr(3) * qu(2))/c    !  sec
c      PPOL(2) =  - (Btr(3) * qu(1) - Btr(1) * qu(3))/c    !  sec
c      PPOL(3) =  - (Btr(1) * qu(2) - Btr(2) * qu(1))/c    !  sec
